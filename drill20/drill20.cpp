...
int main()
{
	...
	int tomb2[10];
	for(int i = 0; i < 10; ++i) tomb2[i] = tomb[i];
	...
	for(auto& i : tomb2) i += 2;

	for(auto& i : vector2) i += 3;

	for(auto& i : lista2) i += 5;
	...
	if(it != vector1.end()) cout << "value found at this position: " << distance(vector1.begin(), it) << endl;
		else cout << "the value is not in the vector\n";

	//////list find//////
	list<int>::iterator itl = find(lista.begin(), lista.end(), 27);
	if(itl != lista.end()) cout << "value found at this position: " << distance(lista.begin(), itl) << endl;
